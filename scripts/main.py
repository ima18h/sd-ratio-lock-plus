...
class RatioLock(scripts.Script):
    ...
    # Our controls
    txt2img_dims_ratio = gr.Dropdown(list(image_ratios.keys()), value="None", label="Image Ratio",
                                     elem_id="txt2img_dims_ratio")
    txt2img_dims_detect = ToolButton(value=detect_ratio_symbol, elem_id="txt2img_dims_detect", label="Detect dims")
    img2img_dims_ratio = gr.Dropdown(list(image_ratios.keys()), value="None", label="Image Ratio",
                                     elem_id="img2img_dims_ratio")
    ...
    def ui(self, is_img2img):
        ...
        width_change_event = self.width.change(fn=self.width_change, inputs=[dims_ratio, self.width, self.height],
                                               outputs=[self.height], show_progress=False)

        # height_change_event = self.txt2img_h_slider.change(fn=height_change, inputs=[dims_ratio, width, height]
        # , outputs=[width], show_progress=False, cancels=[width_change_event])
